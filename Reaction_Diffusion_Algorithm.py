...
class Display:
    def __init__(self):
        ...
        pygame.display.set_caption("Reactio_Diffusion")
        self.stop = False
        self.frame = 0
    ...
    def laplaceA(self, i, j):
        ...
        sum += self.grid[i + 1][j].a * 0.2
        sum += self.grid[i - 1][j].a * 0.2
        sum += self.grid[i][j + 1].a * 0.2
        sum += self.grid[i][j - 1].a * 0.2
        sum += self.grid[i + 1][j - 1].a * 0.05
        sum += self.grid[i + 1][j + 1].a * 0.05
        sum += self.grid[i - 1][j + 1].a * 0.05
        sum += self.grid[i - 1][j - 1].a * 0.05
        return sum
    ...
    def laplaceB(self, i, j):
        ...
        sum += self.grid[i + 1][j].b * 0.2
        sum += self.grid[i - 1][j].b * 0.2
        sum += self.grid[i][j + 1].b * 0.2
        sum += self.grid[i][j - 1].b * 0.2
        sum += self.grid[i + 1][j - 1].b * 0.05
        sum += self.grid[i + 1][j + 1].b * 0.05
        sum += self.grid[i - 1][j + 1].b * 0.05
        sum += self.grid[i - 1][j - 1].b * 0.05
        return sum
